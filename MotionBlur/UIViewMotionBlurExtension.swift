...
extension UIView {
    ...
    var blurLayer: CALayer? {
        ...
        set {
            objc_setAssociatedObject(self, &kUIViewBlurLayer, newValue, UInt(OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        }
    }
    ...
    var displayLink: CADisplayLink? {
        ...
        set {
            objc_setAssociatedObject(self, &kUIViewDisplayLink, newValue, UInt(OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        }
    }
    ...
    var lastPosition: NSValue? {
        ...
        set {
            objc_setAssociatedObject(self, &kUIViewLastPosition, newValue, UInt(OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        }
    }
    ...
    func enableMotionBlur(angle: Float, completion: (Void -> Void)?) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            let blurredImageRef: CGImageRef = CGImageCreateByApplyingMotionBlur(snapshot, angle)
